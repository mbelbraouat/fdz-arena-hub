...
import { LogOut, Trophy, Users, Star, Target, Calendar, Plus, Trash2, Edit2, Save, X, ChevronRight } from 'lucide-react';
import { useQuery, useMutation, useQueryClient } from '@tanstack/react-query';

type Tab = 'tournaments' | 'teams' | 'players' | 'matches' | 'events';
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        ...
        <Button onClick={() => { resetForm(); setEditId(null); setShowForm(true); }} size="sm">
        ...
        <div className="bg-card border border-border rounded-xl p-5 mb-6 space-y-4">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <select value={form.game} onChange={(e) => setForm({ ...form, game: e.target.value as 'cs2' | 'valorant' })} className="w-full h-10 rounded-md border border-input bg-background px-3 text-sm">
            ...
            <div>
              <Label className="text-xs">Teams Count</Label>
              <Input type="number" value={form.teams_count} onChange={(e) => setForm({ ...form, teams_count: parseInt(e.target.value) || 0 })} />
            </div>
            <div>
              <Label className="text-xs">Prize Pool</Label>
              <Input value={form.prize_pool} onChange={(e) => setForm({ ...form, prize_pool: e.target.value })} placeholder="500,000 DZD" />
            </div>
            <div>
              <Label className="text-xs">Stage</Label>
              <Input value={form.stage} onChange={(e) => setForm({ ...form, stage: e.target.value })} placeholder="Group Stage" />
            </div>
            <div>
          ...
          <div>
            ...
            <Textarea value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} placeholder="Tournament description..." />
          </div>
          <div className="flex gap-2">
            <Button onClick={() => saveMutation.mutate()} disabled={saveMutation.isPending} size="sm">
              <Save className="w-4 h-4 mr-1" /> {editId ? 'Update' : 'Create'}
            </Button>
            <Button variant="outline" onClick={() => { setShowForm(false); setEditId(null); }} size="sm">
      ...
      {isLoading ? (
        ...
        <div className="bg-card border border-border rounded-xl overflow-hidden">
          ...
          {tournaments?.length === 0 && <div className="p-6 text-center text-muted-foreground text-sm">No tournaments yet. Add one above.</div>}
        </div>
      )}
    </div>
  );
...
function TeamsPanel() {
  ...
  const { data: teams, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from('teams').select('*').order('rating', { ascending: false });
      if (error) throw error;
      return data;
    },
  });
  ...
  const startEdit = (t: any) => {
    setForm({ name: t.name, tag: t.tag, game: t.game, wins: t.wins, losses: t.losses, rating: t.rating, rank_change: t.rank_change, streak: t.streak || 'W0' });
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        ...
        <Button onClick={() => { setForm({ name: '', tag: '', game: 'cs2', wins: 0, losses: 0, rating: 1.0, rank_change: 0, streak: 'W0' }); setEditId(null); setShowForm(true); }} size="sm">
...
function PlayersPanel() {
  ...
  const { data: players, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from('players').select('*, teams(name, tag)').order('rating', { ascending: false });
      if (error) throw error;
      return data;
    },
  });
  ...
  const { data: teams } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from('teams').select('id, name, tag').order('name');
      if (error) throw error;
      return data;
    },
  });
  ...
  const saveMutation = useMutation({
    mutationFn: async () => {
      const payload = {
        ...
        game: form.game,
        maps_played: Number(form.maps_played),
        kd: Number(form.kd),
        adr: Number(form.adr),
        kast: Number(form.kast),
        hs_percentage: Number(form.hs_percentage),
  ...
  const startEdit = (p: any) => {
    setForm({ name: p.name, team_id: p.team_id || '', game: p.game, maps_played: p.maps_played, kd: p.kd, adr: p.adr, kast: p.kast, hs_percentage: p.hs_percentage, rating: p.rating, is_mvp: p.is_mvp });
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        ...
        <Button onClick={() => { setForm({ name: '', team_id: '', game: 'cs2', maps_played: 0, kd: 1.0, adr: 0, kast: 0, hs_percentage: 0, rating: 1.0, is_mvp: false }); setEditId(null); setShowForm(true); }} size="sm">
      ...
      {showForm && (
        <div className="bg-card border border-border rounded-xl p-5 mb-6 space-y-4">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <div><Label className="text-xs">Maps Played</Label><Input type="number" value={form.maps_played} onChange={(e) => setForm({ ...form, maps_played: parseInt(e.target.value) || 0 })} /></div>
            ...
            <div className="flex items-end gap-2 pb-1">
              <input type="checkbox" checked={form.is_mvp} onChange={(e) => setForm({ ...form, is_mvp: e.target.checked })} className="w-4 h-4" />
              <Label className="text-xs">MVP</Label>
            </div>
          </div>
          <div className="flex gap-2">
            <Button onClick={() => saveMutation.mutate()} disabled={saveMutation.isPending} size="sm"><Save className="w-4 h-4 mr-1" /> {editId ? 'Update' : 'Create'}</Button>
          ...
          {players?.map((p: any) => (
            <div key={p.id} className="grid grid-cols-[1fr_80px_50px_50px_50px_50px_50px_60px_70px] gap-2 px-4 py-2.5 border-b border-border/30 items-center hover:bg-secondary/10 text-sm">
              <div className="flex items-center gap-1.5">
                {p.is_mvp && <Star className="w-3 h-3 text-cs2-gold fill-cs2-gold" />}
...
function MatchesPanel() {
  ...
  const { data: matches, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from('matches').select('*, team1:teams!matches_team1_id_fkey(name, tag), team2:teams!matches_team2_id_fkey(name, tag), tournaments(name)').order('match_date', { ascending: false });
      if (error) throw error;
      return data;
    },
  });
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        ...
        <Button onClick={() => { setForm({ tournament_id: '', team1_id: '', team2_id: '', score1: 0, score2: 0, map: 'TBD', status: 'upcoming', match_date: '' }); setEditId(null); setShowForm(true); }} size="sm">
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        ...
        <Button onClick={() => { setForm({ name: '', status: 'upcoming', teams_count: 0, prize_pool: '', event_date: '', winner: '', game: 'cs2' }); setEditId(null); setShowForm(true); }} size="sm">
      ...
      {isLoading ? <div className="text-muted-foreground text-sm">Loading...</div> : (
        <div className="bg-card border border-border rounded-xl overflow-hidden">
          <div className="grid grid-cols-[1fr_60px_80px_80px_80px_80px_70px] gap-2 px-4 py-2.5 bg-secondary/30 text-[10px] font-semibold text-muted-foreground uppercase tracking-wider border-b border-border">
            <span>Name</span><span>Game</span><span>Status</span><span>Teams</span><span>Prize</span><span>Winner</span><span>Actions</span>
          </div>
          {events?.map((e: any) => (
            <div key={e.id} className="grid grid-cols-[1fr_60px_80px_80px_80px_80px_70px] gap-2 px-4 py-2.5 border-b border-border/30 items-center hover:bg-secondary/10 text-sm">
              ...
              <span className="text-xs text-primary">{e.prize_pool}</span>
